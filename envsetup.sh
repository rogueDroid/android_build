...
EOF
    T=$(gettop)
    for i in `cat $T/build/envsetup.sh $T/vendor/rogue/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      echo "$i"
    done | column
}
...
function build_build_var_cache()
{
    ...
    cached_vars=`cat $T/build/envsetup.sh $T/vendor/rogue/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
    cached_abs_vars=`cat $T/build/envsetup.sh $T/vendor/rogue/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^rogue_") ; then
       CM_BUILD=$(echo -n $1 | sed -e 's/^rogue_//g')
...
function lunch()
{
    ...
    then
        ...
        T=$(gettop)
        pushd $T > /dev/null
        vendor/rogue/build/tools/roomservice.py $product
        popd > /dev/null
        check_product $product
    else
        vendor/rogue/build/tools/roomservice.py $product true
    fi
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/rogue/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
...
. vendor/rogue/tools/variant.sh && ./vendor/rogue/tools/changelog.sh &&
. vendor/rogue/build/envsetup.sh
